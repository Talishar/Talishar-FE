...
if (!deploymentUrl) {
  console.error(
    'Usage: npm run verify:deployment -- https://talishar.net'
  );
  process.exit(2);
}
...
const expectedContentType = (assetName) => {
  if (assetName.endsWith('.js')) return /javascript|ecmascript/i;
  if (assetName.endsWith('.css')) return /^text\/css/i;
  return /^(?!text\/html)/i;
};
...
const verifyAsset = async (assetName) => {
  ...
  try {
    ...
    if (!response.ok) {
      return `${assetName}: HTTP ${response.status}`;
    }
    if (!expectedContentType(assetName).test(contentType)) {
      return `${assetName}: expected its asset MIME type, received ${contentType || 'no Content-Type'}`;
    }
    return null;
  } catch (error) {
